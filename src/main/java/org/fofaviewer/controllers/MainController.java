...
public class MainController {
    ...
    private void initialize() {
        ...
        exportTxtBtn.setText(resourceBundle.getString("EXPORT_TXT_BUTTON"));
    ...
    private void exportTxtAction() {
        ...
        if(file != null){
            ...
            String fileName = file.getAbsolutePath() + System.getProperty("file.separator")
                    + resourceBundle.getString("EXPORT_FILENAME") + System.currentTimeMillis() + ".txt";
            try (FileWriter writer = new FileWriter(fileName)) {
                for(String i : urlList){
                    writer.write(i);
                    writer.write(System.lineSeparator()); // 写入换行符
                }
                System.out.println("数据已成功写入到文件：" + fileName);
            } catch (IOException e) {
